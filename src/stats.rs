...
impl SolverRanking {
    ...
    fn compete(&self, a: &SolverRecord, b: &SolverRecord) -> Ordering {
        for metric in &self.options.metrics {
            match metric {
                Metric::Best => {
                    let av = a.best_values().collect::<Vec<_>>();
                    let bv = b.best_values().collect::<Vec<_>>();
                    if MannWhitneyU::new(av.iter().cloned(), bv.iter().cloned()).test(Alpha::P05) {
                        if average(av.iter().map(|v| v.get())) < average(bv.iter().map(|v| v.get()))
                        {
                ...
                Metric::Auc => {
                    let av = a.aucs().collect::<Vec<_>>();
                    let bv = b.aucs().collect::<Vec<_>>();
                    if MannWhitneyU::new(av.iter().cloned(), bv.iter().cloned()).test(Alpha::P05) {
                        if average(av.iter().map(|v| v.get())) < average(bv.iter().map(|v| v.get()))
                        {
                            return Ordering::Less;
                ...
                Metric::Wallclock => {
                    let av = a.elapsed_times().collect::<Vec<_>>();
                    let bv = b.elapsed_times().collect::<Vec<_>>();
                    if MannWhitneyU::new(av.iter().cloned(), bv.iter().cloned()).test(Alpha::P05) {
                        if average(av.iter().map(|v| v.get())) < average(bv.iter().map(|v| v.get()))
                        {
                            return Ordering::Less;
